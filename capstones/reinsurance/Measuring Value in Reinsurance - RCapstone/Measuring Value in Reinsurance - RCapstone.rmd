...
`r echo = FALSE; {
sep <- 33000000; paste0('$', round(sep/1000000, 0), 'M')
}` 
of a casualty coverage
with an expected loss ratio of 
`r echo = FALSE; {
elr <- 22.9/33; paste0(round(elr * 100, 0), '%')
}` 
...
`r echo = FALSE; {
combinedratio <- elr + expenseratio; paste0(round(combinedratio * 100, 0), '%')
}`.

![Fig 1: Venter's Example Has Two LOBs plus cat -- replace based on your data](VenterFigure1.png "Figure 1")

ABCD is considering two reinsurance programs:

* an excess-of-loss (XOL) programs providing ??x?? of cover for $xxx
* an aggregate stop-loss (ASL) program of ??x?? for a premium of $yyyy. 
...
![Table 1: replace with a graph showing premium trend](VenterTable1.png "Table 1")
from the simulation run shows some summary statistics for 
**NBOR =** net premiums minus losses (gross less ceded) 
...
It shows that the stop loss program is over 
`r '$5M better'` 
...
NBOR = net premium less net losses. 
...
![Fig 2: replace based on your data](VenterFigure2.png "Figure 2")
...
![Fig 3: replace based on your data](VenterFigure3.png "Figure 3")
...
The upper right part shows that the stop loss is indeed more profitable in the good years. 
...
These distributions are shown in table form below. The XOL program better protects the worst case event, but by the 0.25% level (worst case in 400 trials) the stop loss is better. From the 12% to 26% levels, the XOL program is better, by as much as $1,100,000. But in the worse years the stop-loss could be over

```{r}
read.csv("VenterTable2.csv")
```
Table 2

$6,000,000 better than XOL, and the median result is almost $2,000,000 better. As the stop-loss is less costly and usually provides a better result, sometimes dramatically so, it would have to be considered a more useful program for ABCD.

A more careful use of vocabulary is actually appropriate here. 
Even though we would use the above table to say that the stop-loss is $6,350,000 better at the 1-in-100 level, 
the 99th percentile loss event is unlikely to be the same event for the two programs. 
Thus the difference between the programs in the 1-in-100 year gross loss event could be more or less than $6,350,000, as could the 99th percentile of the distribution of the difference between the programs. 
...
In this example the gross loss at this level is $13.85M compared to $0.06M for the stop loss, 
which is a difference of $13.8M in capital needed. 
If capital costs 15%, $2.1M would be needed to raise this much, 
compared to the $1M net cost of the reinsurance. 
...
... Replace Venter's conclusion

## Acknowledgments

Thank you Brian for sending me the link to Gary's *eForum* paper
and thank you Gary for providing a pre-edition doc version ...
and saved me all this typing!